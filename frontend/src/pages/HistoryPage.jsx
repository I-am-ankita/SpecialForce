...
import {
  Clock, Target, Brain, RotateCcw,
  ChevronRight, Trophy, TrendingUp,
  TrendingDown, Calendar, Filter, X
} from "lucide-react";
...
export default function HistoryPage() {
  ...
  const filtered = results.filter(r => {
    ...
    const zoneMatch  = zone === "all" ? true
      : zone === "test"     ? !r.autoSubmitted === false || r.totalTimeTaken > 0
      : zone === "practice" ? r.quizTitle?.toLowerCase().includes("retry") || r.autoSubmitted === false
      : true;
    return topicMatch;
  });
    ...
    <div className="page-container animate-fade">
      ...
      {summary && (
        <div className="grid-4" style={{ marginBottom: "1.5rem" }}>
          {[
            ...
            {
              label: "Avg Accuracy", value: `${summary.avgAcc}%`,
              icon: <Target size={18} color="var(--info)" />,
              color: "var(--info)", bg: "rgba(56,189,248,0.15)",
            },
            {
            ...
            {
              label: "Lowest Score", value: `${summary.worst}%`,
      ...
      <div style={{
        ...
        {ZONE_TABS.map(({ id, label, icon }) => (
          <button key={id} onClick={() => setZone(id)} style={{
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.25rem", flexWrap: "wrap", alignItems: "center" }}>
          ...
          {topics.map(t => (
            <button key={t} onClick={() => setFilterTopic(t)} style={{
              padding: "4px 12px", borderRadius: "100px", fontSize: "0.78rem",
              fontWeight: 600, border: "1px solid var(--border)", cursor: "pointer",
              background: filterTopic === t ? "var(--accent)" : "var(--bg-elevated)",
          ...
          {filterTopic && (
            <button onClick={() => setFilterTopic("")} style={{
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: "0.9rem", color, lineHeight: 1,
        }}>
          {result.accuracy}%
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.4rem", flexShrink: 0 }}>
        {result.difficulty && result.difficulty !== "Mixed" && (
          <span className={`badge ${
            ...
            result.difficulty === "Medium" ? "badge-warning"  : "badge-danger"
